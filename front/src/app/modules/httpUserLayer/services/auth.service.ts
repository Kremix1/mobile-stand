...
import {BehaviorSubject, EMPTY, NEVER, Observable, of, throwError} from 'rxjs';
...
import {IOrgRegisterRequestModel} from "../interfaces/org-register.request-model.interface";
...
export class AuthService {
    ...
    private orgCreateUrl: string = '/organization/create/organization'
    ...
    public registerOrganization(dto: IOrgRegisterRequestModel): Observable<void>{
        return this._identityRequest.post<IUserCredentionalResponseModel, IOrgRegisterRequestModel>({
            url: concatUrl(this._baseHref + this.orgCreateUrl),
            body: dto
        }).pipe(
            tap((response: any) => {
                this.propGuid = response.data?.guid || '';
                this._isSuccessAuth.next(true)
            }),
            mapTo(void 0)
        );
